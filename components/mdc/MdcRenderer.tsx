...
export default function MdcRenderer({ content }: Props) {
  ...
  return (
    <div className="prose prose-neutral dark:prose-invert max-w-none min-w-0 break-words prose-headings:tracking-tight prose-headings:font-semibold prose-h1:text-4xl prose-h2:text-2xl prose-h2:mt-12 prose-h3:text-xl prose-p:leading-relaxed prose-p:text-foreground/85 prose-li:text-foreground/85 prose-pre:rounded-xl prose-pre:border prose-pre:border-border/40 prose-pre:bg-muted/40 prose-pre:text-foreground prose-pre:overflow-x-auto prose-pre:max-w-full [&_pre_code]:bg-transparent [&_pre_code]:text-inherit [&_pre_code]:p-0 prose-code:before:hidden prose-code:after:hidden prose-code:rounded prose-code:bg-muted/60 prose-code:px-1.5 prose-code:py-0.5 prose-code:text-[0.85em] prose-code:font-medium prose-blockquote:border-l-2 prose-blockquote:border-foreground/20 prose-blockquote:not-italic prose-blockquote:text-foreground/80">
      {segments.map((seg, i) => {
        if (seg.kind === "md") {
